  ...
  return (
    ...
    >
      <div className="backdrop-blur-sm bg-white/20 rounded-3xl p-6 border border-white/30 shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-105 hover:bg-white/30 relative overflow-hidden">
        ...
        <div className={`absolute inset-0 bg-gradient-to-r ${gradientFrom}/10 ${gradientTo}/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500`} aria-hidden="true"></div>
        
        <div className="flex items-start space-x-4 mb-4 relative z-10">
          <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${gradientFrom} ${gradientTo} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300 shadow-lg`} aria-label={`Step ${number}`}>
            <span className="text-white font-bold text-lg" aria-hidden="true">{number}</span>
          </div>
          <div>
        ...
        <div className={`absolute bottom-0 left-0 h-1 bg-gradient-to-r ${gradientFrom} ${gradientTo} w-0 group-hover:w-full transition-all duration-700 ease-out`} aria-hidden="true"></div>
      </div>
    </article>
  );
}
